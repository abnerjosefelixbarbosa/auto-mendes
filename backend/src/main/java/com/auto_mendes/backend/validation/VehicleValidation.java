	...
	public void validateVehicle(Vehicle vehicle) {
		...
		if (vehicle.getPlate() != null) {
			if (vehicle.getPlate().length() > 15) {
				throw new BusinessException("Placa deve ter no máximo 15 caracteres.");
			}
			
			if (isExistsPlate) {
				throw new BusinessException("Placa deve não deve ser duplicada.");
			}	
		}
		
		if (vehicle.getPrice().scale() != 2) {
			throw new BusinessException("Preço deve ter 2 dígitos depois da vírgula.");
		}
	}
}