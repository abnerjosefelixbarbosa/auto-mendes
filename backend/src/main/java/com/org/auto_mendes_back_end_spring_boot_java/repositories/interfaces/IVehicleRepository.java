...
public interface IVehicleRepository extends JpaRepository<Vehicle, String> {
	@Query(value = "SELECT v.id, v.plate, v.color, v.vehicle_value, v.vehicle_year, v.exchange_type, m.name FROM"
			+ " vehicles v JOIN models m"
			+ " ON v.id = m.id", nativeQuery = true)
